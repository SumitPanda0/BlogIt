...
class UserTest < ActiveSupport::TestCase
  ...
  def test_user_should_belong_to_organization
    assert_respond_to @user, :organization
  end

  def test_user_should_have_many_posts
  ...
  def test_password_required_method
    ...
    user = User.new(
      ...
      password_digest: BCrypt::Password.create("welcome"),
      organization: create(:organization)
    )
    assert_not user.send(:password_required?)
  end

  def test_password_present_method
    user = User.new(name: "Test User", email: "test@example.com")
    assert_not user.send(:password_present?)

    user.password = "welcome"
    assert user.send(:password_present?)
  end
end
